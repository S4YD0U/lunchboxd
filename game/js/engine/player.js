...
export function loadCharSave(charId) {
  try {
    ...
    if (raw) {
      const saved = Object.assign(DEFAULT_CHAR_SAVE(), JSON.parse(raw));
      // Garantit que les attaques de base sont toujours débloquées,
      // même si une ancienne save ne les contient pas.
      const BASE_ATTACKS = ['punch', 'combo', 'defend', 'special'];
      for (const id of BASE_ATTACKS) {
        if (!saved.unlockedAttacks.includes(id)) saved.unlockedAttacks.push(id);
      }
      // Garantit que le loadout par défaut est présent si vide
      const noSpecial = saved.equippedLoadout.filter(id => id !== 'special');
      if (noSpecial.length === 0) {
        saved.equippedLoadout = ['punch', 'combo', 'defend', 'special'];
      }
      return saved;
    }
...
export function savePlayer() {
  ...
  // Ajoute un timestamp pour la stratégie de fusion cloud
  const dataToSave = Object.assign({}, player, { _savedAt: Date.now() });
  localStorage.setItem('lunchboxe_char_' + selectedHero, JSON.stringify(dataToSave));
  // Sync cloud en arrière-plan (ne bloque pas le jeu)
  cloudSaveChar(selectedHero, dataToSave).catch(() => {});
}

/**
 * Charge la save d'un perso en fusionnant local + cloud.
 * Retourne une Promise — à attendre lors de la sélection d'un héros.
 */
export async function loadCharSaveWithCloud(charId) {
  const local = loadCharSave(charId);
  return mergeAndLoadChar(charId, local, DEFAULT_CHAR_SAVE);
}
...
/**
 * Version async : charge depuis local ET cloud, prend le plus récent.
 * À utiliser lors de la sélection d'un héros.
 */
export async function switchCharSaveWithCloud(charId) {
  const merged = await loadCharSaveWithCloud(charId);
...
export function saveRegionProgress(prog) {
  const key = regionProgressKey();
  localStorage.setItem(key, JSON.stringify(prog));
  localStorage.setItem(key + '_savedAt', String(Date.now()));
  // Sync cloud en arrière-plan
  cloudSaveRegions(selectedHero, prog).catch(() => {});
}

/**
 * Charge et fusionne la progression régions depuis local + cloud.
 * À appeler une fois lors de la sélection d'un héros.
 */
export async function loadRegionProgressWithCloud() {
  const local = loadRegionProgress();
  return mergeAndLoadRegions(selectedHero, local);
}
