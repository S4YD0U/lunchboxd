...
function plantOnPlot(idx){if(!G.selectedSeed){showToast('Sélectionne une graine d\'abord !','bad');return;}const seed=SEEDS.find(s=>s.id===G.selectedSeed);if(!seed)return;if(G.gold<seed.cost){showToast('Pas assez d\'or !','bad');return;}G.gold-=seed.cost;G.plots[idx]={...G.plots[idx],crop:seed.id,plantedAt:G.tick,status:'growing'};updateHUD();renderFarm();addLog(`Planté ${seed.emoji} <b>${seed.name}</b> en parcelle ${idx+1}.`,'skill');}
