...
async function selectHero(id) {
  setSelectedHero(id);
  // Charge depuis le cloud (fusionne avec le local) avant d'afficher
  await switchCharSaveWithCloud(id);
  // Charge aussi la progression des régions depuis le cloud
  const regionProg = await loadRegionProgressWithCloud();
  // Mettre à jour localStorage avec la progression fusionnée
  try {
    localStorage.setItem('lunchboxe_region_progress_' + id, JSON.stringify(regionProg));
...
function renderSkillTree() {
  // Délègue au rendu complet (DOM généré dynamiquement)
  renderSkillTreeFull();
}

window.unlockSkill = function(id) {
  buySkill(id,
    (sk, newLv) => { showToast(`✨ ${sk.emoji} ${sk.name} — niveau ${newLv} !`, 'skill'); renderSkillTreeFull(); updateProfileUI(player, selectedHero, HEROES, getXpForLevel); },
...
window.doResetSkills = function() {
  ...
  if (doResetSkills((msg) => showToast(msg, 'error'))) {
    renderSkillTreeFull(); updateProfileUI(player, selectedHero, HEROES, getXpForLevel);
...
window.equipItem = function(id, type) {
  ...
  if (shopScreen && shopScreen.style.display !== 'none') renderShop();
...
window.openLootboxUI = function(id) {
  if (!selectedHero) { showToast('Sélectionne un héros d\'abord !', 'error'); return; }
  const lb = LOOTBOX_TIERS.find(l => l.id === id);
  if (!lb) { showToast('Boîte introuvable.', 'error'); return; }
  ...
  if (beamsEl) {
    ...
    const cols = tierColors[lb.tier] || tierColors.common;
    for (let i = 0; i < 10; i++) {
  ...
  setTimeout(() => { emoji.style.animation = 'chestShake 0.45s ease'; }, 1300);
  setTimeout(() => { emoji.style.animation = 'chestShake 0.38s ease'; }, 1950);
  setTimeout(() => {
    ...
    setTimeout(() => _revealLootboxItems(result, lb), 550);
  }, 2500);
};

function _revealLootboxItems(result, lb) {
  const openStage   = document.getElementById('lbOpeningStage');
  const revealStage = document.getElementById('lbRevealStage');
  ...
  const hasEpic      = result.items.some(i => i.rarity === 'epic');
  ...
  row.innerHTML = '';
  let revealedCount = 0;
  const totalCards  = result.items.length;
  ...
  result.items.forEach((item, idx) => {
    ...
    let icon, name, stat, isDuplicate = false;
    const DUPLICATE_GOLD_MAP = { common: 10, rare: 30, epic: 80, legendary: 200 };
    if (item.type === 'gold') {
      ...
      icon = item.icon || '❓'; name = item.name || '???'; stat = item.stat || '';
      isDuplicate = !!item.isDupe;
    }
    const dupGold = isDuplicate ? (DUPLICATE_GOLD_MAP[item.rarity] || 10) : 0;
    ...
    card.innerHTML = `
      <div class="lb-card-inner">
        ...
        <div class="lb-card-face rarity-${item.rarity}">
          ${isDuplicate ? `<div class="lb-new-badge" style="background:rgba(245,166,35,0.25);border-color:rgba(245,166,35,0.6);color:var(--gold)">♻️ DOUBLON</div>` : '<div class="lb-new-badge">NOUVEAU</div>'}
    ...
    card.onclick = () => {
      ...
      if (revealedCount >= totalCards) {
        ...
        setTimeout(() => { document.getElementById('lbCloseBtn').classList.add('show'); }, 400);
      }
    };
    ...
    setTimeout(() => { card.style.opacity = '1'; card.style.transform = 'translateY(0)'; }, idx * 120 + 100);
  });

  setTimeout(() => {
    document.getElementById('lbClickHint').style.display = 'block';
  }, result.items.length * 120 + 300);
}
...
function spawnPatateSteem() {
  ...
  for (let i = 0; i < 14; i++) {
    setTimeout(() => {
      ...
      p.style.cssText = `left:${10+Math.random()*30}%;bottom:${5+Math.random()*20}%;width:${size}px;height:${size}px;background:radial-gradient(circle,${colors[Math.floor(Math.random()*colors.length)]} 0%,transparent 70%);animation-duration:${0.9+Math.random()*0.8}s;animation-delay:${Math.random()*0.3}s;`;
...
function triggerPatateSpecial(heroEffective, villain, dmgResult, onComplete) {
  ...
  document.getElementById('patateCinHeroImg').src = '../sprite/' + heroChar.sprite + '.png';
  document.getElementById('patateCinHeroSlamImg').src = '../sprite/' + heroChar.sprite + '.png';
  document.getElementById('patateCinVillainChargeImg').src = '../sprite/' + villain.sprite + '.png';
  document.getElementById('patateCinVillainRecoilImg').src = '../sprite/' + villain.sprite + '.png';
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { document.getElementById('patateCinGround').classList.add('cracking'); }, 900);
    setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { document.getElementById('patatePhaseTitle').classList.add('active'); }, 200);
...
// ─── switchTab avec rendu des onglets ─────────────────────────────────────────
...
function renderShopSection(containerId, items) {
  ...
  items.forEach(item => {
    ...
    let btnAction = '';
    if (!locked) {
      if (item.type === 'consumable')          btnAction = `onclick="buyConsumable('${item.id}')"`;
      else if (equipped)                       btnAction = `onclick="window.unequip('${item.id}','${item.type}')"`;
      else if (owned)                          btnAction = `onclick="window.equipItem('${item.id}','${item.type}')"`;
      else                                     btnAction = `onclick="window.buyItem('${item.id}','${item.type}')"`;
    }
...
function renderLootboxGrid() {
  const el = $('lootboxGrid'); if (!el) return;
  el.innerHTML = '';
  LOOTBOX_TIERS.forEach(lb => {
    const cantAfford = player.gold < lb.price;
    const dropPills = lb.drops.map(d => `<span class="drop-pill rarity-${d.rarity}">${d.label}</span>`).join('');
    const guaranteePills = lb.guarantees.map(g => `<span class="drop-pill rarity-c">✓ ${g}</span>`).join('');
    el.innerHTML += `<div class="lootbox-card tier-${lb.tier}${cantAfford ? ' cant-afford-lb' : ''}" onclick="${cantAfford ? '' : `openLootboxUI('${lb.id}')`}"><span class="lootbox-card-chest">${lb.chest}</span><span class="lootbox-tier-badge">${lb.id==='lb_mythic'?'✨ Mythique':lb.tier==='common'?'🔵 Commun':lb.tier==='rare'?'🟣 Rare':'🟡 Épique'}</span><div class="lootbox-card-name">${lb.name}</div><div class="lootbox-card-desc">${lb.desc}</div><div class="lootbox-card-drops" style="margin-bottom:0.4rem;">${dropPills}</div><div class="lootbox-card-drops">${guaranteePills}</div><div class="lootbox-card-price">🪙 ${lb.price}<span>${cantAfford?`Il te manque ${lb.price-player.gold} 🪙`:`${lb.dropCount} objets garantis`}</span></div></div>`;
  });
}

function renderInventoryShop() {
  const weaponSlot = $('equippedWeapon');
  const armorSlot  = $('equippedArmor');
  ...
  const invEl = $('invSlots'); if (!invEl) return;
  invEl.innerHTML = '';
  for (let i = 0; i < 8; i++) {
...
window.buyConsumable = function(id) {
  ...
  if (player.level < item.reqLv) { showToast('🔒 Niveau ' + item.reqLv + ' requis !', 'error'); return; }
  if (player.gold < item.price)  { showToast('🪙 Pas assez de pièces !', 'error'); return; }
  const total = player.inventory.reduce((s, i) => s + i.qty, 0);
  if (total >= 8) { showToast('🎒 Inventaire plein (8 max) !', 'error'); return; }
  player.gold -= item.price;
  const existing = player.inventory.find(i => i.id === id);
  if (existing && item.stackable) existing.qty++;
  else player.inventory.push({ id, qty: 1 });
  savePlayer(); renderShop(); updateProfileUI(player, selectedHero, HEROES, getXpForLevel);
  showToast(item.icon + ' ' + item.name + ' ajouté à l\'inventaire !', 'success');
};

// ─── Arbre de compétences (rendu complet depuis game.js) ──────────────────────
function renderSkillTreeFull() {
  const spEl = $('skillPointsDisplay'); if (spEl) spEl.textContent = player.skillPoints + ' point(s) disponible(s)';
  const container = $('skillBranches'); if (!container) return;
  container.innerHTML = '';
  for (const branch of SKILL_BRANCHES) {
    ...
    branch.skills.forEach((sk, idx) => {
      ...
      nodeEl.className  = 'skill-node' + (idx > 0 && getSkillLevel(branch.skills[idx-1].id) > 0 ? ' prev-unlocked' : '');
      ...
      const dots = Array.from({length: sk.maxLevel}, (_,i) => {
        ...
        return `<div class="${filled?(sk.type==='active'?'skill-dot filled-active':'skill-dot filled'):'skill-dot'}"></div>`;
      }).join('');
      ...
      const clickAttr = (!maxed && prereqMet) ? `onclick="window.unlockSkill('${sk.id}')"` : '';
...
// ─── Fonctions manquantes pour les onclick HTML ───────────────────────────────
