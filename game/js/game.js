...
const HEROES = {
  fraise: {
    ...
    special: { name: '🍓 Rafale Rouge', fn: (hero) => { const hits=[Math.max(1,Math.floor(hero.atk*0.5+Math.random()*15)),Math.max(1,Math.floor(hero.atk*0.5+Math.random()*15)),Math.max(1,Math.floor(hero.atk*0.6+Math.random()*20))]; return { dmg: hits.reduce((a,b)=>a+b,0), hits, log: '🍓 RAFALE ROUGE — '+hits.join(' + ')+' dégâts !', type: 'multi' }; } },
...
function startBattle(newGame = false) {
  ...
  document.getElementById('heroSprite').innerHTML = `<img src="../sprite/${state.heroChar.sprite}.png" alt="${state.heroChar.name}" />`;
  document.getElementById('villainSprite').innerHTML = `<img src="../sprite/${state.villainChar.sprite}.png" alt="${state.villainChar.name}" />`;
  updateStageIndicators();
...
function openDialogue(villainId, heroId, onComplete) {
  ...
  document.getElementById('dlgHeroSprite').innerHTML = `<img src="../sprite/${hero.sprite}.png" alt="${hero.name}"/>`;
  document.getElementById('dlgVillainSprite').innerHTML = `<img src="../sprite/${villain.sprite}.png" alt="${villain.name}"/>`;
