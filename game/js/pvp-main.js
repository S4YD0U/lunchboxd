...
import { calcDmg, isCrit, getAttacks, getLoadout, getAllAttackDefs } from './engine/battle.js';
import {
...
function injectPvpStyles() {
  ...
  s.textContent = `
    ...
    /* ── Frappe du Goulash — Animation Spéciale ── */
    @keyframes goulash-shake{0%,100%{transform:translateX(0)}10%{transform:translateX(-14px) rotate(-2deg)}20%{transform:translateX(16px) rotate(2.5deg)}30%{transform:translateX(-18px) rotate(-3deg)}40%{transform:translateX(20px) rotate(3deg)}50%{transform:translateX(-16px) rotate(-2deg)}60%{transform:translateX(18px) rotate(2deg)}70%{transform:translateX(-12px) rotate(-1.5deg)}80%{transform:translateX(10px) rotate(1deg)}90%{transform:translateX(-6px)}}
    @keyframes goulash-flash{0%,100%{opacity:0}15%,45%,75%{opacity:1}30%,60%,90%{opacity:.4}}
    @keyframes goulash-title{0%{opacity:0;transform:translate(-50%,-50%) scale(.3) rotate(-8deg)}40%{opacity:1;transform:translate(-50%,-50%) scale(1.15) rotate(2deg)}55%{transform:translate(-50%,-50%) scale(.95) rotate(-1deg)}70%{transform:translate(-50%,-50%) scale(1.05) rotate(.5deg)}100%{opacity:1;transform:translate(-50%,-50%) scale(1) rotate(0deg)}}
    @keyframes goulash-out{0%{opacity:1;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-50%) scale(1.4)}}
    @keyframes goulash-particle{0%{opacity:1;transform:translate(0,0) scale(1) rotate(0deg)}100%{opacity:0;transform:translate(var(--gx),var(--gy)) scale(0) rotate(720deg)}}
    @keyframes goulash-impact{0%{transform:scale(0);opacity:1}60%{transform:scale(2.2);opacity:.8}100%{transform:scale(3.5);opacity:0}}
    @keyframes goulash-crack{0%{opacity:0;transform:translate(-50%,-50%) scale(.5)}20%{opacity:1;transform:translate(-50%,-50%) scale(1.1)}100%{opacity:0;transform:translate(-50%,-50%) scale(1.6)}}
    @keyframes goulash-opp-launch{0%{transform:translateX(0) translateY(0) rotate(0deg)}30%{transform:translateX(40px) translateY(-30px) rotate(15deg)}60%{transform:translateX(-15px) translateY(60px) rotate(-8deg)}80%{transform:translateX(-30px) translateY(20px) rotate(-12deg)}100%{transform:translateX(0) translateY(0) rotate(0deg)}}
    @keyframes goulash-me-charge{0%{transform:translateX(0) scale(1)}30%{transform:translateX(-8px) scale(.92)}60%{transform:translateX(60px) scale(1.15)}80%{transform:translateX(-4px) scale(1)}100%{transform:translateX(0) scale(1)}}
    @keyframes goulash-overlay-in{0%{opacity:0}100%{opacity:1}}
    @keyframes goulash-dmg-num{0%{opacity:0;transform:translate(-50%,-50%) scale(.2)}20%{opacity:1;transform:translate(-50%,-50%) scale(1.4)}40%{transform:translate(-50%,-50%) scale(.9)}60%{transform:translate(-50%,-50%) scale(1.1)}80%{transform:translate(-50%,-50%) scale(1)}100%{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    @keyframes goulash-shockwave{0%{transform:translate(-50%,-50%) scale(0);opacity:.9}100%{transform:translate(-50%,-50%) scale(4);opacity:0}}
    @keyframes goulash-star-spin{0%{transform:translate(-50%,-50%) rotate(0deg) scale(0);opacity:1}50%{transform:translate(-50%,-50%) rotate(180deg) scale(1.3);opacity:1}100%{transform:translate(-50%,-50%) rotate(360deg) scale(0);opacity:0}}
    .goulash-overlay{position:fixed;inset:0;z-index:9999;pointer-events:none;overflow:hidden}
    .goulash-bg{position:absolute;inset:0;background:radial-gradient(ellipse at center,rgba(180,30,10,.85) 0%,rgba(80,10,0,.97) 60%,rgba(0,0,0,.99) 100%);animation:goulash-overlay-in .18s ease-out forwards}
    .goulash-flash{position:absolute;inset:0;background:#fff;animation:goulash-flash .6s ease-out .1s forwards;opacity:0}
    .goulash-title{position:absolute;top:40%;left:50%;transform:translate(-50%,-50%);font-family:'Playfair Display',serif;font-size:clamp(2.2rem,7vw,4.5rem);font-weight:900;color:#fff;text-align:center;white-space:nowrap;letter-spacing:-.02em;text-shadow:0 0 60px rgba(255,100,0,.9),0 0 120px rgba(255,50,0,.6),0 4px 0 rgba(0,0,0,.8);line-height:1.1;animation:goulash-title .5s cubic-bezier(.34,1.56,.64,1) forwards;opacity:0}
    .goulash-title-sub{display:block;font-size:.38em;letter-spacing:.15em;text-transform:uppercase;color:#fbbf24;text-shadow:0 0 30px rgba(251,191,36,.8);margin-top:.2em}
    .goulash-dmg{position:absolute;top:62%;left:50%;transform:translate(-50%,-50%);font-family:'Playfair Display',serif;font-size:clamp(3rem,12vw,7rem);font-weight:900;color:#fbbf24;text-shadow:0 0 40px rgba(251,191,36,.9),0 0 80px rgba(255,100,0,.7),0 6px 0 rgba(0,0,0,.9);animation:goulash-dmg-num .4s cubic-bezier(.34,1.56,.64,1) .55s forwards;opacity:0;white-space:nowrap}
    .goulash-particle{position:absolute;border-radius:50%;pointer-events:none;animation:goulash-particle var(--gd,.8s) ease-out var(--gdelay,0s) forwards}
    .goulash-shockwave{position:absolute;top:50%;left:50%;width:120px;height:120px;border-radius:50%;border:4px solid rgba(255,150,50,.8);animation:goulash-shockwave .7s ease-out .1s forwards}
    .goulash-shockwave2{position:absolute;top:50%;left:50%;width:80px;height:80px;border-radius:50%;border:3px solid rgba(255,255,200,.6);animation:goulash-shockwave .55s ease-out .25s forwards}
    .goulash-crack{position:absolute;top:50%;left:50%;font-size:clamp(4rem,15vw,10rem);animation:goulash-crack .6s ease-out .05s forwards;opacity:0;filter:drop-shadow(0 0 20px #ff6600)}
    /* ── Rafale Rouge (Fraise) ── */
    @keyframes rafale-bg-pulse{0%{opacity:0}8%{opacity:1}20%{opacity:.3}35%{opacity:1}50%{opacity:.2}65%{opacity:1}80%{opacity:.15}100%{opacity:0}}
    @keyframes rafale-slash{0%{opacity:0;transform:translate(-50%,-50%) scaleX(0) rotate(var(--ra))}15%{opacity:1;transform:translate(-50%,-50%) scaleX(1.2) rotate(var(--ra))}40%{opacity:1;transform:translate(-50%,-50%) scaleX(1) rotate(var(--ra))}100%{opacity:0;transform:translate(-50%,-50%) scaleX(1.5) rotate(var(--ra))}}
    @keyframes rafale-title{0%{opacity:0;transform:translate(-50%,-50%) skewX(-15deg) scale(.4)}30%{opacity:1;transform:translate(-50%,-50%) skewX(-8deg) scale(1.1)}50%{transform:translate(-50%,-50%) skewX(-5deg) scale(.97)}100%{opacity:1;transform:translate(-50%,-50%) skewX(-5deg) scale(1)}}
    @keyframes rafale-counter{0%{opacity:0;transform:translate(-50%,-50%) scale(0) rotate(-20deg)}40%{opacity:1;transform:translate(-50%,-50%) scale(1.3) rotate(5deg)}60%{transform:translate(-50%,-50%) scale(.9) rotate(-2deg)}100%{opacity:1;transform:translate(-50%,-50%) scale(1) rotate(0deg)}}
    @keyframes rafale-spark{0%{opacity:1;transform:translate(0,0) scale(1)}100%{opacity:0;transform:translate(var(--sx),var(--sy)) scale(0) rotate(540deg)}}
    @keyframes rafale-me-dash{0%{transform:translateX(0) scaleX(1)}20%{transform:translateX(-12px) scaleX(.85)}45%{transform:translateX(55px) scaleX(1.2)}65%{transform:translateX(35px) scaleX(1.1)}80%{transform:translateX(55px) scaleX(1.2)}100%{transform:translateX(0) scaleX(1)}}
    @keyframes rafale-opp-rattle{0%,100%{transform:translateX(0)}15%{transform:translateX(8px) rotate(3deg)}30%{transform:translateX(-10px) rotate(-4deg)}45%{transform:translateX(12px) rotate(3deg)}60%{transform:translateX(-8px) rotate(-2deg)}75%{transform:translateX(6px) rotate(1deg)}90%{transform:translateX(-4px)}}
    .rafale-overlay{position:fixed;inset:0;z-index:9999;pointer-events:none;overflow:hidden}
    .rafale-bg{position:absolute;inset:0;background:radial-gradient(ellipse at 65% 50%,rgba(220,20,60,.9) 0%,rgba(120,0,30,.96) 55%,rgba(10,0,5,.99) 100%);animation:rafale-bg-pulse 1.6s ease-in-out forwards}
    .rafale-slash{position:absolute;top:50%;left:50%;height:6px;border-radius:3px;background:linear-gradient(90deg,transparent,#fff 30%,#fca5a5 70%,transparent);filter:blur(.5px) drop-shadow(0 0 8px #ef4444);animation:rafale-slash var(--rd,.4s) ease-out var(--rdelay,0s) both;width:clamp(140px,45vw,320px)}
    .rafale-title{position:absolute;top:35%;left:50%;transform:translate(-50%,-50%);font-family:'Playfair Display',serif;font-size:clamp(2rem,6.5vw,4rem);font-weight:900;color:#fff;text-align:center;letter-spacing:-.01em;text-shadow:0 0 50px rgba(220,20,60,.9),0 0 100px rgba(255,0,50,.5),0 4px 0 rgba(0,0,0,.8);line-height:1.1;animation:rafale-title .35s cubic-bezier(.34,1.3,.64,1) .1s both;white-space:nowrap}
    .rafale-title-sub{display:block;font-size:.36em;letter-spacing:.18em;text-transform:uppercase;color:#fca5a5;text-shadow:0 0 20px rgba(252,165,165,.7);margin-top:.2em}
    .rafale-combo{position:absolute;top:58%;left:50%;font-family:'DM Mono',monospace;font-size:clamp(1rem,3.5vw,1.8rem);font-weight:700;color:#fca5a5;letter-spacing:.05em;text-shadow:0 0 20px rgba(220,20,60,.8);animation:rafale-counter .3s cubic-bezier(.34,1.56,.64,1) var(--rdelay,0s) both;white-space:nowrap}
    .rafale-dmg{position:absolute;top:70%;left:50%;font-family:'Playfair Display',serif;font-size:clamp(2.5rem,10vw,5.5rem);font-weight:900;color:#fff;text-shadow:0 0 40px rgba(220,20,60,.9),0 0 80px rgba(255,50,80,.6),0 5px 0 rgba(0,0,0,.9);animation:rafale-counter .35s cubic-bezier(.34,1.56,.64,1) .9s both;white-space:nowrap}
    .rafale-spark{position:absolute;border-radius:50%;pointer-events:none;animation:rafale-spark var(--sd,.5s) ease-out var(--sdelay,0s) both}
    /* ── Onde Juteuse (Poire) ── */
    @keyframes onde-bg-breathe{0%{opacity:0}15%{opacity:1}70%{opacity:.9}100%{opacity:0}}
    @keyframes onde-ring-expand{0%{transform:translate(-50%,-50%) scale(0);opacity:.9}100%{transform:translate(-50%,-50%) scale(var(--os,3));opacity:0}}
    @keyframes onde-orb-float{0%{opacity:0;transform:translate(-50%,-50%) scale(0)}20%{opacity:1;transform:translate(-50%,-50%) scale(1.05)}50%{transform:translate(-50%,-50%) scale(.97)}80%{transform:translate(-50%,-50%) scale(1.03)}100%{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    @keyframes onde-title{0%{opacity:0;transform:translate(-50%,-50%) scale(.5) translateY(20px)}35%{opacity:1;transform:translate(-50%,-50%) scale(1.08) translateY(-4px)}55%{transform:translate(-50%,-50%) scale(.96) translateY(2px)}100%{opacity:1;transform:translate(-50%,-50%) scale(1) translateY(0)}}
    @keyframes onde-heal-float{0%{opacity:0;transform:translate(-50%,-200%) scale(.5)}20%{opacity:1;transform:translate(-50%,-200%) scale(1.1)}60%{opacity:1;transform:translate(-50%,-300%) scale(1)}100%{opacity:0;transform:translate(-50%,-400%) scale(.8)}}
    @keyframes onde-petal{0%{opacity:1;transform:translate(0,0) scale(1) rotate(0deg)}100%{opacity:0;transform:translate(var(--px),var(--py)) scale(0) rotate(480deg)}}
    @keyframes onde-dmg{0%{opacity:0;transform:translate(-50%,-50%) scale(.3)}30%{opacity:1;transform:translate(-50%,-50%) scale(1.15)}50%{transform:translate(-50%,-50%) scale(.92)}100%{opacity:1;transform:translate(-50%,-50%) scale(1)}}
    @keyframes onde-me-glow{0%,100%{filter:brightness(1)}40%{filter:brightness(1.5) drop-shadow(0 0 12px #86efac)}70%{filter:brightness(1.3) drop-shadow(0 0 8px #4ade80)}}
    @keyframes onde-opp-impact{0%{transform:translateX(0) translateY(0)}20%{transform:translateX(18px) translateY(-12px)}45%{transform:translateX(-8px) translateY(18px)}65%{transform:translateX(10px) translateY(-6px)}80%{transform:translateX(-5px) translateY(4px)}100%{transform:translateX(0) translateY(0)}}
    .onde-overlay{position:fixed;inset:0;z-index:9999;pointer-events:none;overflow:hidden}
    .onde-bg{position:absolute;inset:0;background:radial-gradient(ellipse at center,rgba(20,120,60,.88) 0%,rgba(5,60,25,.96) 55%,rgba(0,10,4,.99) 100%);animation:onde-bg-breathe 2s ease-in-out forwards}
    .onde-ring{position:absolute;top:50%;left:50%;border-radius:50%;border:3px solid rgba(134,239,172,.7);animation:onde-ring-expand var(--od,.8s) ease-out var(--odelay,0s) both;width:100px;height:100px}
    .onde-ring-fill{position:absolute;top:50%;left:50%;border-radius:50%;background:radial-gradient(circle,rgba(134,239,172,.25) 0%,transparent 70%);animation:onde-ring-expand var(--od,.9s) ease-out var(--odelay,0s) both;width:100px;height:100px}
    .onde-orb{position:absolute;top:50%;left:50%;width:clamp(100px,28vw,200px);height:clamp(100px,28vw,200px);border-radius:50%;background:radial-gradient(circle at 38% 38%,rgba(220,252,231,.95) 0%,rgba(74,222,128,.8) 40%,rgba(22,163,74,.6) 70%,rgba(5,150,50,.4) 100%);box-shadow:0 0 60px rgba(74,222,128,.6),0 0 120px rgba(34,197,94,.3);animation:onde-orb-float .5s cubic-bezier(.34,1.3,.64,1) .1s both;filter:blur(.5px)}
    .onde-orb-emoji{position:absolute;top:50%;left:50%;font-size:clamp(2.5rem,9vw,5rem);animation:onde-orb-float .45s cubic-bezier(.34,1.56,.64,1) .15s both;filter:drop-shadow(0 0 18px rgba(74,222,128,.9))}
    .onde-title{position:absolute;top:28%;left:50%;font-family:'Playfair Display',serif;font-size:clamp(2rem,6.5vw,4rem);font-weight:900;color:#fff;text-align:center;letter-spacing:-.01em;text-shadow:0 0 50px rgba(34,197,94,.9),0 0 100px rgba(74,222,128,.5),0 4px 0 rgba(0,0,0,.8);line-height:1.1;animation:onde-title .4s cubic-bezier(.34,1.3,.64,1) .2s both;white-space:nowrap}
    .onde-title-sub{display:block;font-size:.36em;letter-spacing:.18em;text-transform:uppercase;color:#86efac;text-shadow:0 0 20px rgba(134,239,172,.7);margin-top:.2em}
    .onde-heal{position:absolute;top:50%;left:50%;font-family:'Playfair Display',serif;font-size:clamp(1.4rem,5vw,2.8rem);font-weight:700;color:#86efac;text-shadow:0 0 30px rgba(74,222,128,.9),0 3px 0 rgba(0,0,0,.7);animation:onde-heal-float 1.2s ease-out .5s both;white-space:nowrap}
    .onde-dmg{position:absolute;top:72%;left:50%;font-family:'Playfair Display',serif;font-size:clamp(2.2rem,8vw,5rem);font-weight:900;color:#fde047;text-shadow:0 0 40px rgba(253,224,71,.8),0 0 80px rgba(250,200,0,.5),0 5px 0 rgba(0,0,0,.9);animation:onde-dmg .4s cubic-bezier(.34,1.56,.64,1) .85s both;white-space:nowrap}
    .onde-petal{position:absolute;pointer-events:none;animation:onde-petal var(--pd,.9s) ease-out var(--pdelay,0s) both}
    /* End */
...
function renderPvpActions(meData) {
  ...
  // On utilise getAllAttackDefs pour récupérer TOUTES les définitions d'attaques,
  // sans filtrer par player.unlockedAttacks (qui est le joueur local, pas le joueur PvP).
  // Le loadout validé est stocké dans Firestore (meData.loadout) — c'est lui qui fait foi.
  const allDefs=getAllAttackDefs(myHero);
  const myLoadout=meData.loadout||['punch','combo','defend','special'];
  const available=allDefs.filter(a=>myLoadout.includes(a.id)||a.id==='special');
  container.innerHTML='';
  const mt=pvp.myTurn;
...
// ─── Animation Frappe du Goulash ─────────────────────────────────────────────
function playGoulashAnimation(dmg) {
  return new Promise(resolve => {
    // Tremblement de l'arène
    const arena = document.querySelector('.pvp-arena') || document.body;
    arena.style.animation = 'goulash-shake .7s ease-out';
    setTimeout(() => { arena.style.animation = ''; }, 700);

    // Animation du sprite de l'attaquant (moi)
    const meEl = $('pvpFighterMe');
    const oppEl = $('pvpFighterOpp');
    if (meEl) { meEl.style.animation = 'goulash-me-charge .6s ease-in-out'; setTimeout(() => meEl.style.animation='', 600); }
    if (oppEl) { setTimeout(() => { oppEl.style.animation = 'goulash-opp-launch .8s ease-in-out .35s'; setTimeout(() => oppEl.style.animation='', 1200); }, 0); }

    // Overlay plein écran
    const overlay = document.createElement('div');
    overlay.className = 'goulash-overlay';

    // Fond rouge sang
    const bg = document.createElement('div');
    bg.className = 'goulash-bg';
    overlay.appendChild(bg);

    // Flash blanc
    const fl = document.createElement('div');
    fl.className = 'goulash-flash';
    overlay.appendChild(fl);

    // Ondes de choc
    const sw1 = document.createElement('div'); sw1.className = 'goulash-shockwave'; overlay.appendChild(sw1);
    const sw2 = document.createElement('div'); sw2.className = 'goulash-shockwave2'; overlay.appendChild(sw2);

    // Crack emoji central
    const crack = document.createElement('div');
    crack.className = 'goulash-crack';
    crack.textContent = '💥';
    overlay.appendChild(crack);

    // Titre de l'attaque
    const title = document.createElement('div');
    title.className = 'goulash-title';
    title.innerHTML = '🥔 FRAPPE DU GOULASH<span class="goulash-title-sub">Colosse du Goulash</span>';
    overlay.appendChild(title);

    // Nombre de dégâts
    const dmgEl = document.createElement('div');
    dmgEl.className = 'goulash-dmg';
    dmgEl.textContent = '−' + dmg;
    overlay.appendChild(dmgEl);

    // Particules (patates, feu, épices)
    const emojis = ['🥔','🥔','🥔','🔥','🔥','🌶️','💥','✨','🍖','🫙'];
    const colors = ['#dc2626','#ea580c','#f97316','#fbbf24','#ef4444','#b91c1c'];
    for (let i = 0; i < 38; i++) {
      const p = document.createElement('div');
      p.className = 'goulash-particle';
      const isEmoji = i < 14;
      const angle = (i / 38) * 360;
      const dist = 80 + Math.random() * 220;
      const gx = Math.cos(angle * Math.PI / 180) * dist;
      const gy = Math.sin(angle * Math.PI / 180) * dist - Math.random() * 80;
      const size = isEmoji ? (1.2 + Math.random() * 1.8) : (6 + Math.random() * 18);
      ...
      p.style.cssText = `
        ...
        font-size:${isEmoji ? size*0.9+'rem' : '0'};
        background:${isEmoji ? 'none' : colors[Math.floor(Math.random()*colors.length)]};
        --gx:${gx}px;--gy:${gy}px;
        --gd:${dur}s;--gdelay:${delay}s;
        margin-left:-${isEmoji?12:size/2}px;margin-top:-${isEmoji?12:size/2}px;
        transform-origin:center;
      `;
      if (isEmoji) p.textContent = emojis[i % emojis.length];
      overlay.appendChild(p);
    }

    // Étoiles de choc (SVG-like avec ✦)
    for (let i = 0; i < 5; i++) {
      ...
      star.style.cssText = `
        ...
        color:#fbbf24;
        animation:goulash-star-spin ${0.5+Math.random()*0.4}s ease-out ${Math.random()*0.3}s forwards;
    ...
    // Nettoyage après l'animation (1.8s total)
    setTimeout(() => {
...
// ─── Animation Rafale Rouge (Fraise) ──────────────────────────────────────────
function playRafaleAnimation(hits) {
  return new Promise(resolve => {
    const meEl = $('pvpFighterMe'), oppEl = $('pvpFighterOpp');
    const totalDmg = hits.reduce((a,b)=>a+b,0);

    // Sprite me : dash vers l'adversaire x3
    if(meEl){ meEl.style.animation='rafale-me-dash .55s ease-in-out'; setTimeout(()=>meEl.style.animation='',560); }

    const overlay = document.createElement('div');
    overlay.className = 'rafale-overlay';

    const bg = document.createElement('div'); bg.className='rafale-bg'; overlay.appendChild(bg);

    // Titre
    const title = document.createElement('div'); title.className='rafale-title';
    title.innerHTML='🍓 RAFALE ROUGE<span class="rafale-title-sub">Guerrière Fruitée</span>';
    overlay.appendChild(title);

    // 3 slashes successifs
    const angles = [-25, 5, -12];
    hits.forEach((dmg, i) => {
      const delay = i * 0.28;

      // Flash rouge
      const fl = document.createElement('div');
      fl.style.cssText=`position:absolute;inset:0;background:rgba(220,20,60,.45);opacity:0;animation:rafale-bg-pulse .22s ease-out ${delay+.05}s both;`;
      overlay.appendChild(fl);

      // Trait de slash
      const slash = document.createElement('div'); slash.className='rafale-slash';
      slash.style.cssText+=`--rd:.4s;--rdelay:${delay}s;--ra:${angles[i]}deg;top:${42+i*8}%;`;
      overlay.appendChild(slash);

      // Emoji strawberry sur l'impact
      const hit = document.createElement('div');
      hit.style.cssText=`position:absolute;top:${38+i*9}%;left:${58+i*4}%;font-size:clamp(1.4rem,5vw,2.5rem);opacity:0;animation:rafale-counter .25s cubic-bezier(.34,1.56,.64,1) ${delay+.1}s both;filter:drop-shadow(0 0 10px rgba(220,20,60,.8));`;
      hit.textContent='🍓';
      overlay.appendChild(hit);

      // Numéro de dégâts individuel
      const num = document.createElement('div'); num.className='rafale-combo';
      num.style.cssText+=`--rdelay:${delay+.12}s;top:${48+i*8}%;left:${63+i*3}%;`;
      num.textContent=`−${dmg}`;
      overlay.appendChild(num);

      // Étincelles
      for(let j=0;j<10;j++){
        const sp=document.createElement('div'); sp.className='rafale-spark';
        const ang=(j/10)*360, dist=30+Math.random()*80;
        sp.style.cssText=`left:${55+i*4}%;top:${45+i*8}%;width:${4+Math.random()*8}px;height:${4+Math.random()*8}px;background:${['#ef4444','#f87171','#fca5a5','#fff'][Math.floor(Math.random()*4)]};--sx:${Math.cos(ang*Math.PI/180)*dist}px;--sy:${Math.sin(ang*Math.PI/180)*dist}px;--sd:${.35+Math.random()*.3}s;--sdelay:${delay+.08+Math.random()*.1}s;margin:-3px;`;
        overlay.appendChild(sp);
      }

      // Sprite adversaire secoué à chaque coup
      if(oppEl){
        setTimeout(()=>{
          oppEl.style.animation='rafale-opp-rattle .35s ease-out';
          setTimeout(()=>oppEl.style.animation='',360);
        }, delay*1000+120);
      }
    });

    // Total
    const dmgEl = document.createElement('div'); dmgEl.className='rafale-dmg';
    dmgEl.style.cssText+='--rdelay:.95s;';
...
// ─── Animation Onde Juteuse (Poire) ──────────────────────────────────────────
function playOndeAnimation(dmg, heal) {
  return new Promise(resolve => {
    const meEl = $('pvpFighterMe'), oppEl = $('pvpFighterOpp');

    // Me : lueur verte
    if(meEl){ meEl.style.animation='onde-me-glow .9s ease-in-out .2s both'; setTimeout(()=>meEl.style.animation='',1150); }

    const overlay = document.createElement('div');
    overlay.className = 'onde-overlay';

    const bg = document.createElement('div'); bg.className='onde-bg'; overlay.appendChild(bg);

    // Anneaux d'onde concentriques
    const ringSizes = [2, 3.5, 5, 7];
    const ringDelays = [0, .15, .3, .45];
    ringSizes.forEach((os, i) => {
      const r = document.createElement('div'); r.className='onde-ring';
      r.style.cssText+=`--os:${os};--od:${.7+i*.1}s;--odelay:${ringDelays[i]}s;margin-left:-50px;margin-top:-50px;`;
      overlay.appendChild(r);
      const rf = document.createElement('div'); rf.className='onde-ring-fill';
      rf.style.cssText+=`--os:${os*.9};--od:${.75+i*.1}s;--odelay:${ringDelays[i]+.05}s;margin-left:-50px;margin-top:-50px;`;
      overlay.appendChild(rf);
    });

    // Orbe central lumineux
    const orb = document.createElement('div'); orb.className='onde-orb';
    orb.style.cssText+='margin-left:calc(clamp(100px,28vw,200px)/-2);margin-top:calc(clamp(100px,28vw,200px)/-2);';
    overlay.appendChild(orb);
    const orbEmoji = document.createElement('div'); orbEmoji.className='onde-orb-emoji';
    orbEmoji.style.cssText+='margin-left:-1.5em;margin-top:-1em;';
    orbEmoji.textContent='🍐';
    overlay.appendChild(orbEmoji);

    // Titre
    const title = document.createElement('div'); title.className='onde-title';
    title.innerHTML='🍐 ONDE JUTEUSE<span class="onde-title-sub">Mage de la Fraîcheur</span>';
    overlay.appendChild(title);

    // Pétales / gouttes qui s'envolent
    const petals = ['🍃','💚','✨','🌿','🍐','💫','🌱'];
    for(let i=0;i<28;i++){
      const p=document.createElement('div'); p.className='onde-petal';
      const angle=(i/28)*360, dist=60+Math.random()*180;
      const size=0.9+Math.random()*1.5;
      p.style.cssText=`left:50%;top:50%;font-size:${size}rem;--px:${Math.cos(angle*Math.PI/180)*dist}px;--py:${Math.sin(angle*Math.PI/180)*dist}px;--pd:${.7+Math.random()*.6}s;--pdelay:${.1+Math.random()*.35}s;margin:-0.75em;`;
      p.textContent=petals[i%petals.length];
      overlay.appendChild(p);
    }

    // Heal flottant (vert clair)
    const healEl = document.createElement('div'); healEl.className='onde-heal';
    healEl.style.cssText+='margin-left:-4em;';
    healEl.textContent=`+${heal} ❤️‍🩹`;
    overlay.appendChild(healEl);

    // Dégâts (jaune)
    const dmgEl = document.createElement('div'); dmgEl.className='onde-dmg';
    dmgEl.style.cssText+='margin-left:-3.5em;';
    dmgEl.textContent=`−${dmg}`;
    overlay.appendChild(dmgEl);
    ...
    if(oppEl){
      setTimeout(()=>{
        oppEl.style.animation='onde-opp-impact .7s ease-out';
        setTimeout(()=>oppEl.style.animation='',720);
      }, 700);
    }
    ...
    setTimeout(()=>{
      overlay.style.transition='opacity .38s ease-out';
      overlay.style.opacity='0';
...
async function handlePvpAction(atk) {
  ...
  if(atk.id==='defend'){
    ...
    // ─ Animations épiques par héros ─
    const heroAnimId = meSnap.heroId;
    if(heroAnimId==='patate'||heroAnimId==='fraise'||heroAnimId==='poire'){
      pvpState.myHp=myHpAfter;pvpState.mySpecial=mySpAfter;pvpState.oppHp=oppHpAfter;
      updatePvpBars();
      // Déclenche l'animation correspondante
      if(heroAnimId==='patate'){
        await playGoulashAnimation(dmgDealt);
      } else if(heroAnimId==='fraise'){
        // Récupère les hits individuels depuis le résultat du spécial
        const hitsArr = r.hits || [Math.round(dmgDealt*0.35), Math.round(dmgDealt*0.32), dmgDealt - Math.round(dmgDealt*0.35) - Math.round(dmgDealt*0.32)];
        await playRafaleAnimation(hitsArr);
      } else if(heroAnimId==='poire'){
        const healAmt = r.heal || 0;
        await playOndeAnimation(dmgDealt, healAmt);
      }
      flashHit('pvpFighterOpp',dmgDealt,heroAnimId==='patate');
      ...
      for(const id in pvpState.attackCooldowns){if(pvpState.attackCooldowns[id]>0)pvpState.attackCooldowns[id]--;if(pvpState.attackCooldowns[id]<=0)delete pvpState.attackCooldowns[id];}
      renderPvpActions(pvp.duelData?.[pvp.role]||meSnap);
      return;
    }
  } else {
